<?php include "header.php";
     ...
     $erini=$ersurname=$eremail=$ercampus=$eruser_type=$erstud_no=$ermy_photo="";
        ...
        if (isset($_POST['register'])) {
           $ini=$surname=$email=$campus=$user_type=$stud_no=$my_photo="";
        if (empty($_POST["ini"])) {
        ...
        if (empty($_POST["stud_no"])) {
          $erstud_no= "Student number is required";
          $Tstud_no=false;
        } else {
          $stud_no = test_input($_POST["stud_no"]);
          $Tstud_no=true;
          
           $staffArr=['216332','223451','229472','226319','219324','236163'];
          if (in_array($stud_no, $staffArr))
              {
              //echo "Match found";
              }
            else
              ...
              $erstud_no ="This staff number is not registered with TUT";
                      $Tstud_no=false;
              }
          
             /* $query="SELECT * FROM tut_database WHERE individual_no='$stud_no' and user='Staff'";
              ...
              $query2="SELECT * FROM student WHERE student_number='$stud_no' AND user_id<>'$id'";
              $result2=mysqli_query($conn,$query2);
               if(!$result2){
                ...
                if($rows2>0){
                      $erstud_no ="This staff number is already registered on this system";
                      $Tstud_no=false;
                  }
       ...
       if (empty($_POST["email"])) {
        ...
        if($rows>0){
         ...
         if ($Tini&&$Tsurname&&$Temail&&$Tcampus&&$Tstud_no) {
            ...
            if(mysqli_query($conn,$sql))
                {
                  ...
                  $get_user_id=mysqli_insert_id($conn);
                    ...
                    // code...
                     ...
                     $sql22="UPDATE `staff` SET `staff_number`='$stud_no' WHERE `user_id`='$id'";
